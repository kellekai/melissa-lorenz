...
! !USES:
  ...
  USE mod_parallel_pdaf, &     ! Parallelization variables fro assimilation
       ONLY: n_modeltasks, task_id, COMM_filter, COMM_couple, filterpe, &
			 npes_filter, mype_filter, MPI_BLK_DECO
  USE mod_assimilation, &      ! Variables for assimilation
       ...
       filename, type_trans, type_sqrt, delt_obs, obs_share, obs_prcnt, &
			 local_dims, state_min_p, state_max_p
...
! *** Screen output ***
  screen2: IF (mype_world == 0) THEN
     WRITE (*, '(1x, a)') 'INITIALIZE MODEL INFORMATION FOR PDAF ONLINE MODE'
     WRITE (*, '(22x,a)') 'MODEL: ...'
