...
!BOP
!
! !ROUTINE: init_ens --- Initialize ensemble for SEIK in offline mode
!
! !INTERFACE:
...
! ********************************
  ...
  do member=1, dim_ens
    if (mype_filter .eq. 0) then
      ens_p(3+member,member) = 1.0
    end if
    seed = 42 + member
    state_p = ens_p(1:dim_p,member)
    call add_noise( state_p, dim_p, 0.02, seed )
    ens_p(1:dim_p,member) = state_p
	end do
